...
import { prepareMissionTreasuryAllocationClaim } from "@/lib/backend/store";
...
export async function POST(request: Request, { params }: { params: Promise<{ missionId: string }> }) {
  try {
    ...
    return ok(await prepareMissionTreasuryAllocationClaim(missionId, { wallet: session.address }));
  } catch (error) {
    return fail(error);
  }
}
